...
function App() {
    ...
    if (window.ethereum) {
      try {
        ...
        if (network.chainId === 5003) {
          ...
          window.alert("Please connect to Mantle Sepolia network");
        }
      } catch (error) {
  ...
  const submitHandler = async (e) => {
    ...
    if (!nft) {
      setMessage(
        "Blockchain connection not established. Please check your network."
      );
      return;
    }
  ...
  const createImage = async () => {
    setMessage("Generating your image ... please hold 🐸✋");
    ...
    try {
      const response = await axios({
        ...
        data: JSON.stringify({
          inputs: `Meme-style: ${description}.
    Create a cute , cartoon style with a humorous and engaging design.`,
  ...
  const uploadToPinata = async (imageData) => {
    setMessage("Uploading your image to the blockchain... 🔥");
    ...
    data.append(
      "file",
      new File([imageData], "image.jpeg", { type: "image/jpeg" })
    );
  ...
  const mintImage = async (tokenURI) => {
    setMessage("Mint your cartoon as an NFT... 🖼️💰");

    try {
      // Additional check to ensure nft contract is loaded
      if (!nft) {
        throw new Error(
          "NFT contract not loaded. Please check your network connection."
        );
      }
      ...
      if (error.code === "INSUFFICIENT_FUNDS") {
        setMessage(
          `Mint failed: Insufficient funds. Please top up your wallet.`
        );
